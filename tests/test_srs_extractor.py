...
def test_tc_c4_nfr_classification():
        ...
        MagicMock(message={
            "content": '''
            {
              ...
              "non_functional": [
                ...
                {
                  "title": "Security",
                  "description": "System shall encrypt user data"
                }
              ]
            }
            '''
    ...
    with patch("service.srs_extractor.generate", return_value=fake_response.choices[0].message["content"]), \
        ...
        result = extractor.extract_requirements("System must be fast and secure")

        nfrs = result["non_functional"]
        ...
        assert nfrs[0]["title"] == "Performance"
        assert nfrs[1]["title"] == "Security"